...
Accepted. rolter will not implement P/D disaggregation; a disaggregated fleet is one upstream and its own coordinator owns phase selection and the KV handoff. The handoff is the whole feature and is not expressible over HTTP — it moves tensor data between accelerators over engine-chosen RDMA-class transports (NIXL/UCX), so placing phases would put an engine-specific, version-coupled binary protocol inside a data plane premised on speaking only the two dominant HTTP dialects, exactly the coupling ADR-0014 prevents. The distinguishing test: does the signal reach rolter over HTTP? Yes for prefix affinity, queue depth, KV-event residency, LMCache occupancy and LoRA-adapter affinity; no for a KV cache transfer. Consequence: disaggregated fleets work with rolter today for any engine, with no code and no mid-request-handoff failure modes; revisit only if an engine exposes phase-aware placement over HTTP.

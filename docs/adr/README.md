...
Accepted. Resolve each virtual key owner's merged access-profile policy when the snapshot is built, carry it on the key record, and enforce it on the gateway's model and route selection; the key's own allow-list and the owner's policy are ANDed. Rejects key-mint-time resolution because a later policy edit would never reach issued keys, so a revocation would silently fail to revoke. Adds `bump_config_version()` triggers to exactly the four tables that feed the resolution, changing migration 0058's premise rather than its rule. Consequence: the policy `/rbac/effective` reports is the policy enforced, and a deployment with no access profiles is unaffected.

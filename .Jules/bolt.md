...
## 2026-08-29 - [JSON Bulk Insertion Performance]
**Learning:** When generating large NDJSON bodies for ClickHouse bulk inserts (like `INSERT INTO ... FORMAT JSONEachRow`), using `serde_json::to_string` and pushing to a `String` inside a loop incurs significant allocation overhead due to intermediate string creation and UTF-8 validation. Since `reqwest::RequestBuilder::body` naturally accepts `Vec<u8>`, we can bypass strings entirely.
**Action:** Use `Vec::with_capacity` for the buffer and stream rows directly using `serde_json::to_writer` to drastically reduce allocations and improve throughput. Similarly, for single payloads, prefer `serde_json::to_vec` over `to_string` when the destination accepts raw bytes.

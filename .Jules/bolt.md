...
## 2026-08-21 - Format macro overhead in iterations
**Learning:** Using `format!` inside an iterator to produce a String (e.g., `bytes.iter().map(|b| format!("{b:02x}")).collect::<String>()`) is extremely slow because it forces parsing the format string and doing heap allocations per element before concatenating them. Extracting this to a single buffer with `String::with_capacity` and doing character insertion via `char::from_digit` is significantly faster.
**Action:** When hex encoding bytes or performing simple string mapping in a loop, avoid `format!` per element. Use `String::with_capacity` and bitwise arithmetic if possible, and extract this logic into shared helpers (like `rolter_auth::hex_encode`) to avoid duplicating this pattern.

...
## 2026-08-04 - JSON Array Serialization to ClickHouse
**Learning:** When serializing JSON array elements into NDJSON (Newline Delimited JSON) for ClickHouse via `reqwest`, appending to a `String` sequentially using `serde_json::to_string(&row)` allocates a temporary `String` per row and copies the buffer. Using `serde_json::to_writer(&mut Vec<u8>, row)` directly skips both the intermediary strings and the copy, reducing latency under load. Crucially, the buffer handles failure elegantly if an error is caught before pushing the `b'\n'`.
**Action:** When creating batch HTTP request bodies (especially for analytics systems like ClickHouse), always allocate a `Vec::with_capacity` and serialize rows directly with `to_writer` instead of building a `String`.

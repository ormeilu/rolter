    ...
    <aside className="overflow-hidden rounded-xl border border-[color:var(--border-subtle)] bg-[color:var(--surface-raised)]">
      ...
      <div className="max-h-[26rem] overflow-y-auto p-1.5 lg:max-h-[calc(100vh-14rem)]">
        {skills.length === 0 ? <p className="px-2 py-5 text-center text-xs text-muted-foreground">No skills yet</p> : skills.map((skill) => (
          <button key={skill.id} type="button" aria-current={selectedId === skill.id ? "page" : undefined} onClick={() => onSelect(skill.id)} className={cn("w-full rounded-lg px-2.5 py-2 text-left transition-colors", selectedId === skill.id ? "bg-[color:var(--surface-selected)]" : "text-[color:var(--text-secondary)] hover:bg-[color:var(--surface-hover)] hover:text-foreground")}>
...
function SourceOption({ selected, title, description, onSelect }: { selected: boolean; title: string; description: string; onSelect: () => void }) {
  return <button type="button" role="radio" aria-checked={selected} onClick={onSelect} className={cn("rounded-lg border p-3 text-left transition-colors", selected ? "border-[color:var(--red-folk)] bg-[color:var(--surface-selected)]" : "border-[color:var(--border-subtle)] hover:bg-[color:var(--surface-hover)]")}><span className="flex items-center gap-2 text-sm font-medium"><span className={cn("flex h-4 w-4 items-center justify-center rounded-full border", selected ? "border-[color:var(--red-folk)]" : "border-[color:var(--border-default)]")}>{selected && <span className="h-2 w-2 rounded-full bg-[color:var(--red-folk)]" />}</span>{title}</span><span className="ml-6 mt-1 block text-xs text-muted-foreground">{description}</span></button>;
}
